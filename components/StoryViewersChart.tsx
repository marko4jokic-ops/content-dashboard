...
import type { StoryDailyPoint } from "@/lib/windsor";
import { fmtCompact, fmtFull, fmtShortDate } from "@/lib/format";

function ChartTooltip({ active, payload }: TooltipProps<number, string>) {
  if (!active || !payload || !payload.length) return null;
  const d = payload[0]?.payload as StoryDailyPoint | undefined;
  if (!d) return null;
  return (
    <div className="rounded-lg border border-edge-strong bg-[#0c0c0e] px-3 py-2 shadow-glass">
      ...
      <p className="text-[11px] text-dim">
        {fmtFull(Math.round(d.avgViewsPerFrame))} avg views / frame
      </p>
    </div>
  );
}

export default function StoryViewersChart({ days }: { days: StoryDailyPoint[] }) {
  if (!days.length) {
    return (
      <div className="flex h-[260px] items-center justify-center text-sm text-faint">
        No story viewer history yet.
      </div>
    );
  }
  ...
  return (
    <div className="h-[260px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          <defs>
            <linearGradient id="storyGoldArea" x1="0" y1="0" x2="0" y2="1">
